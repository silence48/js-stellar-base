...
function checkFastSigningNode() {
  ...
  actualMethods.generate = function (secretKey) {
    var pk = _BrowserBuffer["default"].alloc(sodium.crypto_sign_PUBLICKEYBYTES);
    var sk = _BrowserBuffer["default"].alloc(sodium.crypto_sign_SECRETKEYBYTES);
    sodium.crypto_sign_seed_keypair(pk, sk, secretKey);
    return pk;
  };
  actualMethods.sign = function (data, secretKey) {
    data = _BrowserBuffer["default"].from(data);
    var signature = _BrowserBuffer["default"].alloc(sodium.crypto_sign_BYTES);
...
function checkFastSigningBrowser() {
  ...
  actualMethods.generate = function (secretKey) {
    var secretKeyUint8 = new _BrowserBuffer["default"](secretKey);
